...
public class OneMagazineIssueView extends OneDocumentView {
    ...
    public GridPane get_one_magazine_issue_view(String main_title_text, MagazineIssue current_magazine_issue){
        ...
        this.issue_number_spinner = new Spinner<>(1, 1000, 1);
        ...
        // Pre-filling values if modifying an existing magazine issue
        if (current_magazine_issue != null) {
            // Setting current magazine if known
    ...
    protected boolean magazine_issue_validity_checks(Connection conn, boolean adding_magazine, MagazineIssue issue_to_change){ // If we are adding a magazine we need to check that its issue number is unique for this magazine 
        boolean error_is_present = false;
        error_is_present = !super.validity_checks(conn);
        ...
        if (this.magazine == null) {
            ...
            this.magazine_combo.setEffect(UIStyling.red_glow);
            ...
            for (Document doc: ApplicationFX.getDocuments()){
                if (doc instanceof MagazineIssue issue){
                    if (issue.getMagazine().equals(this.magazine) && issue.getIssue_number() == this.issue_number){
                        main_error_message += "Please enter a unique issue number for this magazine!\n";
                        error_is_present = true;
                        this.issue_number_spinner.setEffect(UIStyling.red_glow);
                        break;
                    }
                }
            }
        ...
        if (this.issue_date == null || this.issue_date.isAfter(LocalDate.now())){
            ...
            error_is_present = true;
            this.issue_date_picker.setEffect(UIStyling.red_glow);
        }
