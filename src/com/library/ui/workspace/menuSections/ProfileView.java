...
public class ProfileView {
    // To hold the original information about the connected user
    private Map<Object, Object> originalValues = new HashMap<>();
    ...
    private void update_original_values(Member connected_member, TextField first_nameField, TextField last_nameField, DatePicker birth_datePicker, TextField addressField, TextField phoneField, TextField mailField) {
        ...
        this.originalValues.put(first_nameField, connected_member.getFirst_name());
        this.originalValues.put(last_nameField, connected_member.getLast_name());
        this.originalValues.put(birth_datePicker, connected_member.getBirth_date());
        this.originalValues.put(addressField, connected_member.getAddress());
        this.originalValues.put(phoneField, connected_member.getPhone_number());
        this.originalValues.put(mailField, connected_member.getMail());
    }

    // To check whether the user has modifyed some information in the fields
    private boolean isAnyFieldModified() {
        for (Object fieldObj : this.originalValues.keySet()) {
            if (fieldObj instanceof TextField tf) {
                if (!tf.getText().equals(this.originalValues.get(tf))) {
                    return true;
                }
            } else if (fieldObj instanceof DatePicker dp) {
                if (!dp.getValue().equals(this.originalValues.get(dp))) {
                    return true;
                }
            }
        }
    ...
    public ScrollPane start(Connection conn) { 
        ...
        // Separating line (separating regular information and password changing)
        ...
        // Message which will display errors in case the user badly filled fields
        Label message = new Label();

        // Buttons
        Button saveButton = new Button("Save modifications");
        Button resetButton = new Button("Reset");
        resetButton.setDisable(true); // initially nothing to reset (if the user has modified something he will be able to click on the button to reset)
        saveButton.setDisable(true); // initially nothing to save (if the user has modified something he will be able to click on the button to save modifications)
        ...
        UIStyling.button_styling(resetButton, "#7b1d26", "white");
        UIStyling.button_styling(saveButton, "#253721", "white");

        // Storing original user's information
        ...
        // Green glow effect for fields that were modified
        ...
        // Attaching listeners to detect modifications of variables
        ChangeListener<String> textListener = (obs, oldVal, newVal) -> {
            ...
            if (!newVal.equals(this.originalValues.get(tf))) {
                tf.setEffect(green_glow);
            } else {
                tf.setEffect(null);
            }
            resetButton.setDisable(!isAnyFieldModified()); // If anything is changed we enable resetting, otherwise we are disabling it
            saveButton.setDisable(!isAnyFieldModified());
        };

        ChangeListener<LocalDate> dateListener = (obs, oldVal, newVal) -> {
            if (!newVal.equals(this.originalValues.get(birth_datePicker))) {
        ...
        resetButton.setOnAction(e -> {
            first_nameField.setText((String) this.originalValues.get(first_nameField));
            last_nameField.setText((String) this.originalValues.get(last_nameField));
            addressField.setText((String) this.originalValues.get(addressField));
            phoneField.setText((String) this.originalValues.get(phoneField));
            mailField.setText((String) this.originalValues.get(mailField));
            birth_datePicker.setValue((LocalDate) this.originalValues.get(birth_datePicker));

            // Removing all glows and error message
        ...
        // Saving is everything is correct, or at least those fields that were correctly filled
        saveButton.setOnAction(e -> {
            ...
            if (!first_name.equals((String) this.originalValues.get(first_nameField))) {
                if (first_name.equals("")) {
                    error_message += "First name cannot be empty\n";
                    first_nameField.setEffect(UIStyling.red_glow);
                } else {
            ...
            if (!last_name.equals((String) this.originalValues.get(last_nameField))) {
                if (last_name.equals("")) {
                    error_message += "Last name cannot be empty\n";
                    last_nameField.setEffect(UIStyling.red_glow);
                } else {
            ...
            if (!birthdate.equals((LocalDate) this.originalValues.get(birth_datePicker))) {
            ...
            if (!address.equals((String) this.originalValues.get(addressField))) {
                connected_member.setAddress(address);
            }

            // phone number can be empty
            String phone = phoneField.getText();
            if (!phone.equals((String) this.originalValues.get(phoneField))) {
            ...
            if (!old_pass.equals("")) {
                if (LoginView.authenticate((String) this.originalValues.get(mailField), old_pass, conn) != null){
                    String new_pass = new_passField.getText();
                    String confirm_pass = confirmField.getText();
                    if (!new_pass.equals(confirm_pass)) {
                        error_message += "Passwords don't match\n";
                        new_passField.setEffect(UIStyling.red_glow);
                        confirmField.setEffect(UIStyling.red_glow);
                    ...
                    old_passField.setEffect(UIStyling.red_glow);
                }
            }

            String mail = mailField.getText();
            if (!mail.equals((String) this.originalValues.get(mailField))) {
                if (mail.equals("")) {
                    error_message += "Email cannot be empty\n";
                    mailField.setEffect(UIStyling.red_glow);
                } else { // We have to check that it is unique in the database
                    if (LibraryManager.exists(conn, "members", "mail", mail)){  // There is already a user with this email address
        ...
        // Wrapping the GridPane inside a ScrollPane, so that the user can scroll
