...
public abstract class OneDocumentView {
    ...
    public GridPane get_one_document_view(String main_title_text, Document current_document){
        ...
        for(Genre g: Genre.values()){
            possible_genres.add(Genre.convertGenreToString(g));
        }
        ...
        // --- Function to create a single author grid ---
        Consumer<Author> add_author_form = (Author author) -> {
        ...
        // --- Button actions ---
        add_author_button.setOnAction(ev -> add_author_form.accept(null));
            ...
            if (selected != null && !selected.isBlank()) {
                ...
                if (!already_added) {
                    ...
                    if (found_author != null){
                        add_author_form.accept(found_author);
                    }
                }
            }
        ...
        if (current_document != null){
            ...
            for(Genre genre: current_document.getGenre()){
                Label genre_chip = new Label(Genre.convertGenreToString(genre) + " ✕");
                genre_chip.setStyle("-fx-background-color: #dfe6e9; -fx-padding: 5 10; -fx-background-radius: 10; -fx-cursor: hand;");
                genre_chip.setOnMouseClicked(ev -> selected_genres_pane.getChildren().remove(genre_chip));
            ...
            for(Author author: current_document.getAuthors()){
                add_author_form.accept(author);
            }
        }

        // --- Add everything to grid except buttons as they will come at the end of everything ---
        int row = 0;
    ...
    protected boolean validity_checks(Connection conn, DropShadow red_glow){ 
